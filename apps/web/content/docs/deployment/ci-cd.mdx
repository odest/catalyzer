...
|---|---|---|
| `ci.yml` | PR to `master` | Format/Lint check, type check, build validation |
...
| `publish-cli.yml` | Release published (`create-catalyzer@v*`) | Publish CLI to npm |
...
Three checks must pass: **formatting & linting** (`pnpm check`), **type checking**, and **production build**.
...
```json title="release-please-config.json (excerpt)"
{
  ...
  "packages": {
    ...
    "packages/cli": {
      ...
      "component": "create-catalyzer"
    }
  }
}
```

When a release is created, all packages bump to the same version, including `package.json`, `tauri.conf.json`, and `Cargo.toml`. The CLI (`create-catalyzer`) is versioned independently with `separate-pull-requests: true`, so it gets its own Release PR.
...
`publish-cli.yml` triggers automatically when a release tagged `create-catalyzer@v*` is published. It builds the CLI with `pnpm --filter create-catalyzer build` and publishes to npm with `--provenance` for supply chain security.
