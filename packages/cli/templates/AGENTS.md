...
<!-- TODO: Describe what your project does in 1-2 sentences. -->

Cross-platform app built with Next.js 16 and Tauri 2 in a pnpm monorepo. The web app (`apps/web`) runs with server-side rendering, a Fumadocs documentation site, and Serwist PWA support. The native app (`apps/native`) takes the same Next.js codebase but builds it as a static HTML export, which Tauri 2 loads into a system webview. Both apps import their pages, hooks, stores, and layout components from `packages/core`, and their design system primitives from `packages/ui`. A change to a component in `packages/ui` shows up everywhere.
